...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Helper to send exactly 1 single summary alert message per product/color variant to a target phone
    ...
    async function sendGroupedAlerts(itemsList, targetPhone, stockTypeTitle) {
      ...
      for (const groupKey of Object.keys(groups)) {
        if (global._activeSendingLocks.has(groupKey)) continue;
        global._activeSendingLocks.add(groupKey);
        ...
        global._activeSendingLocks.delete(groupKey);
